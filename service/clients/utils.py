...
class GooglelAccountAdapter(DefaultSocialAccountAdapter):
    def populate_user(self, request, sociallogin, data):
        user = super().populate_user(request, sociallogin, data)
        user.is_verified = True  # Устанавливаем флаг верификации
        return user
...
def validate_username(email):
    if SocialAccount.objects.filter(user__email=email).exists():
        raise forms.ValidationError(
            "Ця адреса електронної пошти зареєстрована через соціальну мережу. Будь ласка, увійдіть через відповідну соціальну мережу.")
